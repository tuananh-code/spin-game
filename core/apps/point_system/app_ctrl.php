...
function cl_get_information($args = array()) {
	global $db, $cl, $me;
    ...
    $limit  = $args['limit'];
	$offset  = $args['offset'];
	$sql    = cl_sqltepmlate('apps/point_system/sql/fetch_information', array(
		't_info' => T_POINTS_INFO,
		'user_id'  => $me['id'],
		'offset'  => $offset,
		'limit'  => $limit
	));
	$information = $db->rawQuery($sql);
	$data   = array();
	$update = array();


	if (cl_queryset($information)) {
...
function cl_get_distris($args = array())
{
	global $db, $cl, $me;
	...
	$limit  = $args['limit'];
	$offset  = $args['offset'];
	$sql    = cl_sqltepmlate('apps/point_system/sql/fetch_distris', array(
		't_distris' => T_POINTS_DISTRI,
		'point_id'  => $args['id'],
		'offset'  => $offset,
		'limit'  => $limit
	));
...
function cl_get_information_details($id)
{
	global $db, $cl, $me;

	$sql_query     = cl_sqltepmlate('apps/point_system/sql/fetch_details', array(
		't_info' => T_POINTS_INFO,
		'user_id'  => $me['id'],
		'id_info'  => $id
	));
...
function cl_get_total_information() {
	global $db, $cl, $me;

	$sql_query     = cl_sqltepmlate('apps/point_system/sql/fetch_total', array(
		't_info' => T_POINTS_INFO,
		'user_id'  => $me['id'],
	));
...
function cl_get_list_stores()
{
	global $db, $cl, $me;

	$sql_query     = cl_sqltepmlate('apps/manage/sql/fetch_stores', array(
		't_stores' => T_STORE,
		'user_id'  => $me['id'],
	));

	$stores = $db->rawQuery($sql_query);
	$data   = array();

	if (cl_queryset($stores)) {
